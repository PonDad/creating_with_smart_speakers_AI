...
@ask.launch
def launched():
    return question('チン、チン、チンアナ〜ゴ。チンアナゴとニシキアナゴを見分けたい時はハイチーズって言ってね')
...
def what():
    ...
    if apple_predict < 0.5:
        ...
        return statement('これは{}ですね。チン、チン、チンアナ〜ゴ。'.format(apple_labels[0]))

    elif apple_predict >= 0.5:
        print(apple_labels[1], apple_predict[0][0])
        return statement('これは{}ですね。チン、チン、チンアナ〜ゴじゃなかった。'.format(apple_labels[1]))
    else:
        print('なにアナゴかよくわかりません')
        return statement('なにアナゴかよくわかりません')
